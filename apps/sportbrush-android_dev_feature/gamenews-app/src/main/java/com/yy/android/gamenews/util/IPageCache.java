...
import com.yy.android.gamenews.util.db.Database;
...
public class IPageCache {
	...
	public boolean set(String key, String content, long cacheTime) {
		...
		String id = db.getValue("select id from " + TABLE + " where id = ?",
				key);
		if (db.errCode != 0) {
			return false;
		}
	...
	public String get(String key) {
		checkDataBaseState();
		HashMap<String, String> result = db.getOneRow(
	...
	public long getRowCreateTime(String key) {
		checkDataBaseState();
		HashMap<String, String> pResult = db.getOneRow(
	...
	public String getNoDelete(String key) {
		checkDataBaseState();
		HashMap<String, String> result = db.getOneRow("select content from "
				+ TABLE + " where id = ?", key);
