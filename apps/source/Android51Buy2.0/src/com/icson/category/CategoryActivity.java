...
public class CategoryActivity extends BaseActivity implements
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if (model instanceof CategoryModel) {
			mNavBar.setText( ((CategoryModel)model).getName());
			currentCategory = (CategoryModel) model;
			currentModles.clear();

			currentModles.addAll(currentCategory.getSubCategorys());

			adapter.notifyDataSetChanged();
			setNavBarText(currentCategory.getName());
...
//				param.putString(ListActivity.REQUEST_PAGE_TITLE, node.name);
	...
	private String getCacheKey() {
		return CacheKeyFactory.CACHE_BLOCK_CATEGORY + "_"
				+ String.valueOf(ILogin.getSiteId());
	}
