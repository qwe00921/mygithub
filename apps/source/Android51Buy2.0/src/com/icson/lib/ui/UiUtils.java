...
import com.icson.R;
import com.icson.lib.ui.AddressRadioDialog.OnAddressRadioSelectListener;
import com.icson.lib.ui.RadioDialog.OnRadioSelectListener;
...
public class UiUtils {
	...
	public static void makeSlotToast(Context context, int headImgRid,String strTitle, String strText) {
		if( null == context || TextUtils.isEmpty(strTitle) ||
				TextUtils.isEmpty(strText))
			return ;
		
		LayoutInflater pInflater = (LayoutInflater)context.getApplicationContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		View pLayout = pInflater.inflate(R.layout.toast_slot_layout, null);
		if ( null != pLayout )
		{
			// Update the text.
			ImageView pImg = (ImageView)pLayout.findViewById(R.id.toast_img);
			pImg.setImageResource(headImgRid);
			
			TextView pTitle = (TextView)pLayout.findViewById(R.id.toast_title);
	...
	public static AddressRadioDialog showAddressListDialog(Context aContext, String strCaption,  String[] options, int checkedItem, OnAddressRadioSelectListener aListener, boolean isCanCancel) {
		return showAddressListDialog(aContext, strCaption, options, checkedItem, aListener, isCanCancel, true);
	}
	
	
	public static AddressRadioDialog showAddressListDialog(Context aContext, String strCaption,  String[] options, int checkedItem, OnAddressRadioSelectListener aListener, boolean isCanCancel, boolean isShowArrow) {
		if(null == aContext || null == options || 0 >= options.length) {
			return null;
		}
		
		AddressRadioDialog pDialog = new AddressRadioDialog(aContext, null, null, isShowArrow);
		pDialog.setCanceledOnTouchOutside(true);
		pDialog.setCancelable(isCanCancel);
		pDialog.setProperty(strCaption);
		pDialog.setList(options, checkedItem);
		pDialog.setAddressRadioSelectListener(aListener);
	...
	public static void showSoftInput(Context aContext, EditText aEditText) {
		if( null != aContext && null != aEditText ) {
			...
			aEditText.requestFocus(); //如果输入框没有获得焦点，先获取焦点
