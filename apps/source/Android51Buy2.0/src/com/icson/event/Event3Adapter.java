...
public class Event3Adapter extends BaseAdapter implements DestroyListener,
		ImageLoadListener, OnClickListener, Runnable {
	public Event3Adapter(BaseActivity activity,
			ArrayList<EventProductModel> dataSource,int aPayType) {

		mInflater = LayoutInflater.from(activity);
		mProductModels = dataSource;
		payType = aPayType;
		// mCount = (int) Math.ceil(mProductModels.size() / 3.0f);
	...
	public View getView(int position, View convertView, final ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.event_3_item, null);
			holder = new ItemHolder();
			holder.container1 = convertView
					.findViewById(R.id.event_3_container_1);
			holder.image1 = (ImageView) convertView
					.findViewById(R.id.event_3_image_1);
			holder.title1 = (TextView) convertView
					.findViewById(R.id.event_3_title_1);
			holder.price1 = (TextView) convertView
					.findViewById(R.id.event_3_price_1);

			holder.container2 = convertView
					.findViewById(R.id.event_3_container_2);
			holder.image2 = (ImageView) convertView
					.findViewById(R.id.event_3_image_2);
			holder.title2 = (TextView) convertView
					.findViewById(R.id.event_3_title_2);
			holder.price2 = (TextView) convertView
					.findViewById(R.id.event_3_price_2);

			holder.container3 = convertView
					.findViewById(R.id.event_3_container_3);
			holder.image3 = (ImageView) convertView
					.findViewById(R.id.event_3_image_3);
			holder.title3 = (TextView) convertView
					.findViewById(R.id.event_3_title_3);
			holder.price3 = (TextView) convertView
					.findViewById(R.id.event_3_price_3);
		...
		EventProductModel model1 = position < mProductModels.size() ? mProductModels
				.get(position) : null;

		EventProductModel model2 = (position + 1) < mProductModels.size() ? mProductModels
				.get(position + 1) : null;

		EventProductModel model3 = (position + 2) < mProductModels.size() ? mProductModels
				.get(position + 2) : null;
		...
		if (model1 != null) {
			...
			holder.price1.setText("¥"
					+ ToolUtil.toPrice(model1.getShowPrice(), 2));
			holder.container1.setTag(model1);
			holder.container1.setTag(R.layout.event_3_item, position);
			holder.container1.setOnClickListener(this);
		}
		holder.container1.setVisibility(model1 == null ? View.INVISIBLE
				: View.VISIBLE);
		...
		if (model2 != null) {
			...
			holder.price2.setText("¥"
					+ ToolUtil.toPrice(model2.getShowPrice(), 2));
			holder.container2.setTag(model2);
			holder.container2.setTag(R.layout.event_3_item, position + 1);
			holder.container2.setOnClickListener(this);
		}

		holder.container2.setVisibility(model2 == null ? View.INVISIBLE
				: View.VISIBLE);
		...
		if (model3 != null) {
			...
			holder.price3.setText("¥"
					+ ToolUtil.toPrice(model3.getShowPrice(), 2));
			holder.container3.setTag(model3);
			holder.container3.setTag(R.layout.event_3_item, position + 2);
			holder.container3.setOnClickListener(this);
		}

		holder.container3.setVisibility(model3 == null ? View.INVISIBLE
				: View.VISIBLE);
	...
	public void onLoaded(Bitmap image, String url) {
		if (0 >= mLatestTimetag) {
			...
			mLatestTimetag = System.currentTimeMillis();
			this.stopPending();
		} else {
			final long nCurrentTimetag = System.currentTimeMillis();
			if (nCurrentTimetag - mLatestTimetag < mNotifyDelayMs) {
				if (!mNotifyPending) {
					mNotifyPending = true;

					// Make sure the pending request
					this.requestPending();
				}
			} else {
				mNotifyPending = false;
	...
	private void requestPending() {
		...
		this.stopPending();
		mHandler.postDelayed(this, mNotifyDelayMs);
	}
	...
	public void onClick(View v) {
		...
		if (pTag != null) {
			...
			if (pModel.getChannelId() > 0) {// 手机多价商品
			...
			String pageId = "";
			if (activity instanceof Event3Activity) {
				pageId = "1990" + (70 + ((Event3Activity) activity).getEventId());
			}

			// 01011
			String locationId = "";
			int id = (Integer) v.getTag(R.layout.event_3_item);
	...
	// Improvement for fast scroll.
	private static final long mNotifyDelayMs = 500;
	private static final int mColumnSize = 3;
	private boolean mNotifyPending = false;
	private long mLatestTimetag = 0;
