...
public class CheckBox extends UiBase {
	...
	protected void parseAttrs(Context pContext, TypedArray array) {
		// Parse attributes.
		isChecked = UiUtils.getBoolean(pContext, array, R.styleable.djcityattrs_isChecked, true);
		mImageOnId = UiUtils.getResId(pContext, array, R.styleable.djcityattrs_checkboxImageOn, 0);
		mImageOffId = UiUtils.getResId(pContext, array, R.styleable.djcityattrs_checkboxImageOff, 0);
		mTextContent = UiUtils.getString(pContext, array, R.styleable.djcityattrs_text);
		mTextColor = UiUtils.getColor(pContext, array, R.styleable.djcityattrs_contentColor);
		mTextSize = UiUtils.getDimension(pContext, array, R.styleable.djcityattrs_contentSize);
	}
