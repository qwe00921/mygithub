...
public class TextField extends UiBase {
	...
	protected void parseAttrs(Context aContext, TypedArray aArray) {
		// Parse attributes.
		mCaptionString = UiUtils.getString(aContext, aArray, R.styleable.djcityattrs_caption);
		mContentString = UiUtils.getString(aContext, aArray, R.styleable.djcityattrs_text);
		mInfoString = UiUtils.getString(aContext, aArray, R.styleable.djcityattrs_info);
		mDrawableId = UiUtils.getResId(aContext, aArray, R.styleable.djcityattrs_drawableRight);
		mColor = UiUtils.getColor(aContext, aArray, R.styleable.djcityattrs_contentColor);
		mGravity = UiUtils.getInteger(aContext, aArray, R.styleable.djcityattrs_contentGravity);
		mContentSize = UiUtils.getDimension(aContext, aArray, R.styleable.djcityattrs_contentSize);
		mCaptionSize = UiUtils.getDimension(aContext, aArray, R.styleable.djcityattrs_captionSize);
