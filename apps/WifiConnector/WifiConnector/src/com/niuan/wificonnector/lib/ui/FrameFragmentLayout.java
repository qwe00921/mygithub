...
public class FrameFragmentLayout extends RelativeLayout {
	...
	private void init(Context context) {
		...
		mFooter.setBackgroundColor(0xaaffffff);
	...
	public void add(List<FrameFragmentItem> itemList) {
		...
		for (final FrameFragmentItem item : itemList) {
			...
			item.btn.setTitle(item.name);
			item.btn.setOnClickListener(new OnClickListener() {
			...
			mFooter.addView(item.btn, params);
	...
	public void setOnFrameChangeListener() {

	}

	public interface OnFrameChangeListener {
		public void onChange(int index, Fragment fragment);
	}
}
